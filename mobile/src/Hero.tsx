...
import { Button, colors, Icon, IconButton, s, useWide } from './ui';
...
export function Hero({ choose, mangoSeason }: { choose: (category: Category | 'all') => void; mangoSeason: () => void }) {
  ...
  const reveal = useRef(new Animated.Value(1)).current, progress = useRef(new Animated.Value(0)).current;
  ...
  return <View style={h.hero}>
    <FruitVideo playing={!paused && !reduced && focused && active} />
    <View style={[h.inner, !wide && { flexDirection: 'column', padding: 22, paddingTop: 30, gap: 8 }]}>
      <Animated.View style={[h.copy, { width: wide ? '52%' : '100%', opacity: reveal, padding: wide ? 28 : 20 }]}>
      ...
      <View style={[h.art, { width: wide ? '44%' : '100%', maxWidth: wide ? 470 : 340, height: wide ? 440 : 290 }]}>
